...
- Смена идёт через LDAP (`IADsUser.ChangePassword`), при сбое — фолбэк на
  `WinNT://ДОМЕН/пользователь`. В редком случае, когда LDAP-смена прошла, но
  ответ потерялся, фолбэк может сообщить «неверный текущий пароль» при том,
  что смена фактически состоялась, — тогда стоит проверить вход новым паролем.
- `Set-Clipboard $null` очищает текущий буфер обмена, но **не** очищает
  историю буфера Windows 10/11 (Win+V) — скопированный пароль останется в ней.
  Если требуется строгая гарантия, историю нужно отключить политикой.
